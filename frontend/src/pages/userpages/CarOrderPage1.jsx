...
import React, { useState, useEffect } from 'react';
import { differenceInDays } from 'date-fns';
import { Box, Image, Text, Heading, Flex, Input, Button, useToast } from '@chakra-ui/react';
...
const CarOrderPage1 = () => {
  ...
  const handleNext = async () => {
    if (pickupDate && returnDate) {
      try {
        const response = await axiosInstance.post('/cart/check-car-availability', {
          carId:carId,
          pickedAt:pickupDate,
        ...
        if (response.data.available) {
          // Navigate to the next page if the car is available
          navigate('/user/orderpage', {
            state: { totalAmount, carimage, carId, pickupDate, returnDate },
          });
        } else {
          // Show a toast notification if the car is unavailable
          toast({
            title: 'Car not available',
            description: `The car is already booked until ${response.data.availableUntil}`,
            status: 'error',
            duration: 5000,
            isClosable: true,
          });
        ...
        toast({
          title: 'Error',
          description: 'Unable to check car availability. Please try again later.',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
