...
const AdditionalRequirementsPage = () => {
    const rows = [
        { id: 1, name: 'Need Baby Seat', details: 'Suitable for babies up to 12 months old.', cost: 500 },
        { id: 2, name: 'Need GPS', details: "We'll guarantee there's GPS navigation in your rental vehicle.", cost: 500 },
        { id: 3, name: 'Need Additional Driver', details: 'Want to share the driving? Add other drivers.', cost: 20 },
    ];
    ...
    const handleToggleChange = (facility, cost) => {
        setFacilities((prevFacilities) => {
            const newState = !prevFacilities[facility];
            const updatedFacilities = { ...prevFacilities, [facility]: newState };
    ...
    return (
        <Box p="4" maxW="1200px" mx="auto">
            ...
            <Stack spacing="4">
                {rows.map(row => (
                    <Box key={row.id} borderWidth="1px" borderRadius="lg" p="4">
                        <Flex justifyContent="space-between" alignItems="center">
                            <Text fontSize="lg">{row.name}</Text>
                            <Flex align="center">
                                <Link href="#" onClick={() => handleRowClick(row.id)} color="blue.500" fontSize="md" mr="2">
                                    {clickedRowId === row.id ? 'Close' : 'Details'}
                                </Link>
                                <Switch isChecked={facilities[row.name.toLowerCase().replace(' ', '')]} onChange={() => handleToggleChange(row.name.toLowerCase().replace(' ', ''), row.cost)} />
                            </Flex>
                        </Flex>
                        <Text mt="2" fontSize="md" color="gray.600">Amount: ${row.cost}/day</Text>
                        {clickedRowId === row.id && (
                            <div align="center">
                                {row.details}
                            </div>
